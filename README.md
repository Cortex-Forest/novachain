...
## Docker 部署（GHCR 自动构建）
镜像：`ghcr.io/cortex-forest/novachain`。镜像默认尝试启用抗量子签名（liboqs-python + Dilithium5，构建期预编译）；若构建环境无法编译抗量子库则自动回退 Ed25519（`/api/status` 如实显示 `quantum_safe` 状态）。含 TLS 自签证书自动生成（`docker-entrypoint.sh`）、状态持久化与健康检查。
