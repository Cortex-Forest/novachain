...
LARGE_THRESHOLD_USD = 100_000.0  # 大额跨链阈值（USD）
LARGE_DELAY = 24 * 3600          # 大额延迟 24 小时
UNBOND = 7 * 86400
...
class Bridge:
    ...
    def _check_limit(self, usd) -> bool:
        return self._daily_used_usd() + usd <= self._daily_limit_usd()
    ...
    def _deposit_apply(self, tx, d):
        ...
        if op == "nova:bridge:deposit":
            ...
            dep = {
                ...
                "fee": fee, "usd": _amt(usd), "large": large,
                "status": "pending", "sigs": [], "created_at": time.time(),
                "available_at": time.time() + LARGE_DELAY if large else 0.0,
            }
